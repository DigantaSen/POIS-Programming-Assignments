		...
		<div className="step-list">
			{steps.map((step, index) => (
				<article
					...
					className={`step-card${step.implemented ? "" : " pending"}`}
				>
...
export default function App() {
	const [activeTab, setActiveTab] = useState<AppTab>("pa16");
	...
	const buildPanel = (
		<article className="panel" aria-label="Build panel">
			<h3>
				Column 1: Build ({foundation} to {buildTarget})
			</h3>
			<p className="panel-note">
				Chain from selected foundation to source primitive with full step trace.
			</p>
			{buildResult.message ? (
				<div className="warn">{buildResult.message}</div>
			) : null}
	...
	const reductionPanel = (
		<article className="panel" aria-label="Reduce panel">
			<h3>
				Column 2: Reduce ({reductionSource} to {reductionTarget})
			</h3>
			<p className="panel-note">
				Reduction consumes the concrete {reductionSource} instance as a
				black-box oracle.
			</p>
			{route === null ? (
				<div className="warn">
					No direct path from {reductionSource} to {reductionTarget} in this
					direction.
			...
			{route === null ? (
				<article className="step-card pending">
					<div className="step-head">
						<strong>
							{reductionSource} to {reductionTarget} (placeholder)
						</strong>
						<span className="status-pill">
							Not implemented yet (due: {primitiveDuePa[reductionTarget]})
						</span>
					</div>
					<p className="kv">
						No direct reduction path is modeled for this direction. Use backward
						mode or implement an additional reduction theorem.
					</p>
				</article>
			) : null}
			{route && route.length === 0 ? (
				<div className="warn">
					Source and target are the same primitive. Reduction is identity.
				</div>
			) : null}
			...
			{route && route.length > 0 && !buildResult.oracle ? (
				<div className="warn">
					Build panel did not produce a usable source oracle.
				</div>
			) : null}
		</article>
	);
	...
	return (
		<main className="app-shell">
			...
			{activeTab === "pa17" && <Pa16To20Panel assignment="pa17" />}

			{activeTab === "pa18" && <Pa16To20Panel assignment="pa18" />}

			{activeTab === "pa19" && <Pa16To20Panel assignment="pa19" />}

			{activeTab === "pa20" && <Pa16To20Panel assignment="pa20" />}
			...
			{activeTab === "pa00" && (
				<>
					<section className="hero">
						...
						<p>
							Interactive scaffold with foundation toggle, two-column
							Build/Reduce pipeline, bidirectional mode, and formal proof
							summary stubs.
						</p>
						...
						<div className="control-grid">
							<div className="control-card">
								...
								<p className="panel-note">
									{foundationModule.name} capabilities:{" "}
									{foundationModule.capabilities.join(", ")}
								</p>
							</div>
							...
							<div className="control-card">
								...
								<div className="control-field">
									<label htmlFor="primitive-a">
										Primitive A (source in forward mode)
									</label>
									<select
										id="primitive-a"
										value={primitiveA}
										onChange={(event) =>
											setPrimitiveA(event.target.value as PrimitiveId)
										}
									>
								...
								<div className="control-field">
									<label htmlFor="primitive-b">
										Primitive B (target in forward mode)
									</label>
									<select
										id="primitive-b"
										value={primitiveB}
										onChange={(event) =>
											setPrimitiveB(event.target.value as PrimitiveId)
										}
									>
										{primitiveOrder.map((primitive) => (
											<option key={primitive} value={primitive}>
							...
							<div className="proof-body">
								...
								{proofSummary.steps.length > 0 ? (
									proofSummary.steps.map((step, index) => (
										<div
											key={`${step.theorem}-${index}`}
											className={`proof-item${step.implemented ? "" : " pending"}`}
										>
											<p className="kv">
												Step {index + 1} theorem: {step.theorem}
											</p>
											...
											<p className="kv">
												...
												{step.implemented
													? ""
													: ` - Not implemented yet (due: ${step.duePa ?? "TBD"})`}
											</p>
										</div>
									))
								) : (
									<div className="warn">
										No reduction theorem chain is available for this direction
										yet.
									</div>
								)}
							</div>
